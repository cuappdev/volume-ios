    ...
    class ViewModel: ObservableObject {
        ...
        func fetchFlyersPage() async {
            ...
            Network.client.queryPublisher(
                query: VolumeAPI.GetFlyersByOrganizationSlugQuery(
                    limit: GraphQLNullable<Double>(floatLiteral: Constants.pageSize),
                    slug: organization.slug,
                    offset: GraphQLNullable<Double>(floatLiteral: Double(flyers?.count ?? 0))
                )
            )

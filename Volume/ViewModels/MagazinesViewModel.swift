    ...
    class ViewModel: ObservableObject {
        ...
        private func fetchFeaturedMagazines() async {
            Network.client.queryPublisher(
                query: VolumeAPI.GetFeaturedMagazinesQuery(
                    limit: Constants.featuredMagazinesLimit
                )
            )
        ...
        private func fetchMagazineSemesters() async {
            Network.client.queryPublisher(
                query: VolumeAPI.GetAllMagazineSemestersQuery(
                    limit: Constants.semesterCountLimit
                )
            )
            .compactMap { $0.data?.magazines.map(\.semester) }
            .sink { [weak self] completion in
                self?.networkState?.handleCompletion(screen: .reads, completion)
        ...
        private func fetchSemesterMagazines() async {
            ...
            Network.client
                .queryPublisher(
                    query: VolumeAPI.GetMagazinesBySemesterQuery(

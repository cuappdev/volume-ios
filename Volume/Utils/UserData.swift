...
class UserData: ObservableObject {
    ...
    func set(article: Article, isSaved: Bool, bookmarkRequestInProgress: Binding<Bool>) async {
        ...
        if isSaved {
            cancellables[.bookmarkArticle(article)] = Network.client.mutationPublisher(
                mutation: VolumeAPI.BookmarkArticleMutation(
                    uuid: uuid, articleID: article.id
                )
            )
            .sink { completion in
    ...
    func set(magazine: Magazine, isSaved: Bool, bookmarkRequestInProgress: Binding<Bool>) async {
        ...
        if isSaved {
            cancellables[.bookmarkMagazine(magazine)] = Network.client.mutationPublisher(
                mutation: VolumeAPI.BookmarkMagazineMutation(
                    uuid: uuid, magazineID: magazine.id
                )
            )
            .sink { completion in
                if case let .failure(error) = completion {
    ...
    func set(flyer: Flyer, isSaved: Bool, bookmarkRequestInProgress: Binding<Bool>) async {
        ...
        if isSaved {
            cancellables[.bookmarkFlyer(flyer)] = Network.client.mutationPublisher(
                mutation: VolumeAPI.BookmarkFlyerMutation(uuid: uuid, flyerID: flyer.id)
            )
            .sink { completion in
                if case let .failure(error) = completion {
    ...
    func set(publication: Publication, isFollowed: Bool, followRequestInProgress: Binding<Bool>) async {
        ...
        if isFollowed {
            cancellables[.followPublication(publication)] = Network.client.mutationPublisher(
                mutation: VolumeAPI.FollowPublicationMutation(
            ...
            cancellables[.unfollowPublication(publication)] = Network.client.mutationPublisher(
                mutation: VolumeAPI.UnfollowPublicationMutation(
                    slug: publication.slug,
                    uuid: uuid
                )
            )
    ...
    func set(organization: Organization, isFollowed: Bool, followRequestInProgress: Binding<Bool>) async {
        ...
        if isFollowed {
            cancellables[.followOrganization(organization)] = Network.client.mutationPublisher(
                mutation: VolumeAPI.FollowOrganizationMutation(
            ...
            cancellables[.unfollowOrganization(organization)] = Network.client.mutationPublisher(
                mutation: VolumeAPI.UnfollowOrganizationMutation(
                    slug: organization.slug,
                    uuid: uuid
                )
            )
